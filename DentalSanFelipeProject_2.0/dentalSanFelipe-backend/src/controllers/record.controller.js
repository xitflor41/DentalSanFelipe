...
export const getExpedientes = async (req, res, next) => {
  try {
    ...
    if (q) {
      baseSql += " WHERE p.nombre LIKE ? OR p.apellido LIKE ? OR e.observaciones LIKE ?";
      const like = `%${q}%`;
...
export const getExpedienteById = async (req, res, next) => {
  try {
    ...
    // consultas vinculadas (por id_expediente)
...
/** PUT /api/records/:id - actualizar expediente (observaciones, id_usuario, fechaVisita, vinculos) */
export const updateExpediente = async (req, res, next) => {
  try {
    const id = parseInt(req.params.id);
    const { id_usuario, fechaVisita, observaciones, id_historiaClinica, id_consulta } = req.body;
    if (!id) return res.status(400).json({ message: "Id inválido" });

    const [result] = await pool.query(
      `UPDATE expedientes SET id_usuario = ?, fechaVisita = ?, observaciones = ?, id_historiaClinica = ?, id_consulta = ?, updated_at = CURRENT_TIMESTAMP WHERE id_expediente = ?`,
      [id_usuario || null, fechaVisita || null, observaciones || null, id_historiaClinica || null, id_consulta || null, id]
    );
    if (result.affectedRows === 0) return res.status(404).json({ message: "Expediente no encontrado" });

    await insertAudit(id, req.user?.id_usuario, "MODIFICAR_EXPEDIENTE", { id_usuario, fechaVisita, observaciones, id_historiaClinica, id_consulta });
...
export const deleteExpediente = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query("DELETE FROM expedientes WHERE id_expediente = ?", [id]);
    if (result.affectedRows === 0) return res.status(404).json({ message: "Expediente no encontrado" });

    await insertAudit(id, req.user?.id_usuario, "ELIMINAR_EXPEDIENTE", {});
